...
func Setup() *cli.Command {
	return &cli.Command{
		...
		Action: func(c *cli.Context) error {
			...
			if err := directoryValidation(cfg.DESTINATION_ROOT_PATH); err != nil {
				...
				cfg.DESTINATION_ROOT_PATH = destinationDir
			...
			if err := fileValidation(cfg.LOG); err != nil {
				...
				cfg.LOG = logPath
				fmt.Printf("%v was set as a destination directory", cfg.DESTINATION_ROOT_PATH)
			}
			cfg.Version = c.App.Version
			fmt.Println("config:")
			fmt.Println("version    :", cfg.Version)
			fmt.Println("source     :", cfg.SOURCE_ROOT_PATH)
			fmt.Println("destination:", cfg.DESTINATION_ROOT_PATH)
