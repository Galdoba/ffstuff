...
func Exists(program string) bool {
	confDir, file := StdConfigPath(program)
...
func ConstructManual(program string) (File, error) {
	dir, file := StdConfigPath(program)
...
func Filepath(program string) string {
	dir, name := StdConfigPath(program)
	return dir + name
}
...
func (f *File) Write(dt []byte) error {
	...
	fl, err := os.OpenFile(f.location, os.O_WRONLY|os.O_APPEND, 0660)
	if err != nil {
		return err
	}
	n, err := fl.Write(dt)
...
func ReadFrom(program string) ([]byte, error) {
	confDir, confFile := StdConfigPath(program)
	f, err := os.OpenFile(confDir+confFile, os.O_RDONLY, 0600)
...
func StdConfigPath(program string) (string, string) {
	home, err := os.UserHomeDir()
	if err != nil {
		fmt.Println(err)
	}
	pathSep := filepath.Separator
	configDir := home + string(pathSep) + ".config" + string(pathSep) + program + string(pathSep)
